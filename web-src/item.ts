...
function findItem(day: DayData, id: string): Item | null {
...
(async () => {
  const title = must<HTMLElement>("#itemTitle");
  const content = must<HTMLElement>("#itemContent");
  const back = must<HTMLAnchorElement>("#backLink");
