...
document.querySelectorAll<HTMLElement>("[data-go]").forEach((a) => {
  a.addEventListener("click", (e) => {
    e.preventDefault();
    const me = e as MouseEvent;
    ripple(me.clientX, me.clientY);
    const target = a.dataset.go;
    if (target) setTimeout(() => go(target), 90);
  });
});
...
// ---------- 后端数据契约（与 Go internal/contract 对齐）----------
interface Counts { commits: number; notes: number; papers: number; }
interface IndexEntry { date: string; counts: Counts; level: number; }
interface Item { id: string; source: string; title: string; detail: string; tags?: string[]; }
...
function total(c: Counts): number { return c.commits + c.notes + c.papers; }
function pad2(n: number): number | string { return n < 10 ? "0" + n : n; }
...
function setStreak(id: string, v: number): void {
  const e = document.getElementById(id);
  if (e) e.innerHTML = `${v}<small>天</small>`;
}

function buildHeatmap(index: IndexEntry[]): void {
  const byDate = new Map<string, IndexEntry>();
  for (const e of index) byDate.set(e.date, e);
    ...
    for (let d = 0; d < 7; d++) {
      ...
      if (dt <= today) {
        ...
        if (cnt > 0) {
          activeDays++;
          sum += cnt;
          if (dt.getMonth() === today.getMonth() && dt.getFullYear() === today.getFullYear()) monthCount++;
        }
        seq.push(cnt);
      } else {
  ...
  if (note) note.textContent = index.length ? `真实数据 · 累计 ${activeDays} 天` : "暂无数据";
}

// ---------- 记录时间线 ----------
function reportSummary(md: string): { title: string; note: string } {
  const lines = md.split(/\r?\n/).map((l) => l.trim()).filter((l) => l !== "");
  if (lines.length === 0) return { title: "", note: "" };
...
function logItem(entry: IndexEntry, day: DayData | null): string {
  const dateDots = entry.date.replace(/-/g, ".");
  const n = total(entry.counts);
  const { title, note } = reportSummary(day?.report ?? "");

  const chips: string[] = [];
  if (entry.counts.commits) chips.push(`git ${entry.counts.commits}`);
  if (entry.counts.notes) chips.push(`obsidian ${entry.counts.notes}`);
  if (entry.counts.papers) chips.push(`zotero ${entry.counts.papers}`);
...
async function buildLog(index: IndexEntry[]): Promise<void> {
  ...
  const rows = await Promise.all(
    active.map(async (e) => ({ e, day: await loadJSON<DayData>(`./data/days/${e.date}.json`) })),
  );
...
async function buildBlog(): Promise<void> {
  ...
  const tagSel = document.getElementById("blogTag") as HTMLSelectElement | null;
  const searchEl = document.getElementById("blogSearch") as HTMLInputElement | null;
  if (!listEl || !tagSel || !searchEl) return;

  const articles = (await loadJSON<Article[]>("./data/articles.json")) ?? [];
  ...
  const opt = (label: string, val: string): string =>
...
// ---------- 关于我：随笔 / 想法 ----------
function musingCard(m: Musing): string {
...
async function buildMusings(): Promise<void> {
  const listEl = document.getElementById("musingList");
  const catSel = document.getElementById("musingCat") as HTMLSelectElement | null;
  if (!listEl || !catSel) return;

  const musings = (await loadJSON<Musing[]>("./data/musings.json")) ?? [];
  if (musings.length === 0) {
  ...
  const opt = (label: string, val: string): string => `<option value="${escapeHtml(val)}">${escapeHtml(label)}</option>`;
...
// ---------- 启动：读数据 → 渲染 ----------
(async () => {
  const index = (await loadJSON<IndexEntry[]>("./data/index.json")) ?? [];
  buildHeatmap(index);
